<script lang="ts">
	...
	import { canStepToNextDay, canStepToPrevDay } from '$lib/utils';
	...
	const prevDate = $derived(date.subtract({ days: 1 }));
	const nextDate = $derived(date.add({ days: 1 }));
...
<span class="stepper" data-sveltekit-preload-data="tap">
	{#if canGoPrev}
		<a
			class="step"
